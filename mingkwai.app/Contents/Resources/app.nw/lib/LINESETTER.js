...
(function() {
  ...
  this.set_lines = function(source, test_line, send_line, done) {
    ...
    html_parser.end();
    return stream.pipe($join_text_nodes()).pipe($hyphenate()).pipe($collect_tags()).pipe(D2.$show()).pipe($split_texts()).pipe($distribute_lines(test_line, send_line, done)).pipe(D2.$on_end((function(_this) {
