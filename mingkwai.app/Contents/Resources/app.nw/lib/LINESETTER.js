...
(function() {
  var $, $async, CND, D, HOTMETAL, XCSS, after, alert, badge, debug, echo, glyph_replacements, help, info, later, log, now, rpr, sleep, step, suspend, urge, warn, whisper;
  ...
  this.demo = function(app, md, settings, handler) {
    var BD, MKTS, XXX_t0, XXX_times, arity, as_html, document, format, gcolumn, gcolumn_left, gcolumn_offset, gcolumn_top, input, jQuery, mark_chrs, mark_lines, matter, mm_from_npx, mm_from_rpx, npx_from_mm, ref, rpx_from_mm, target_column, try_slug, window, zoomer, ƒ;
    debug('©E054j', 'demo');
    ...
    input.pipe(as_html).pipe($((function(_this) {
      ...
      return function(block_hotml, send) {
        var container, excess, excesses, good_slug_html, html_lines, is_finished, last_line_block_tag, last_line_hotml, last_line_html, last_start_idx, line_count, line_nr, new_class, ref1, slice_hotml, slices, slug_html, start_idx, stop_idx, trial_count;
        start_idx = 0;
        stop_idx = 0;
        trial_count = 0;
        ...
        if (container.length === 0) {
          container = jQuery("<container style='display:block'></container>");
        }
        ...
        while (!is_finished) {
          ...
          ref1 = try_slug(container, block_hotml, line_nr, start_idx, stop_idx), slice_hotml = ref1[0], slug_html = ref1[1], line_count = ref1[2], excess = ref1[3];
      ...
      return function(html_lines, send) {
        var html_line, i, j, len, len1;
        for (i = 0, len = html_lines.length; i < len; i++) {
          html_line = html_lines[i];
          whisper(html_line);
        }
        for (j = 0, len1 = html_lines.length; j < len1; j++) {
          html_line = html_lines[j];
  ...
  this.demo_v1 = function(app, md, settings, handler) {
    var BD, MKTS, arity, as_html, document, format, gcolumn, gcolumn_left, gcolumn_offset, gcolumn_top, input, jQuery, live, mark_chrs, mark_lines, matter, mm_from_npx, mm_from_rpx, npx_from_mm, ref, rpx_from_mm, t0, window, zoomer, ƒ;
    debug('©E054j', 'demo');
    switch (arity = arguments.length) {
      case 3:
        handler = settings;
