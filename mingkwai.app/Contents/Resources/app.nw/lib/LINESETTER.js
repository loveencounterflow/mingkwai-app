...
(function() {
  var $collect_tags, $collect_texts, $distribute_lines, $hyphenate, $split_texts, CND, D2, Htmlparser, LineBreaker, TEACUP, after, alert, badge, debug, echo, help, hyphenate, info, log, new_html_parser, njs_fs, njs_path, partition_text, remit, render_close_tag, render_open_tag, rpr, sleep, step, suspend, urge, warn, whisper,
  ...
  this.set_lines = function(source, test_line, send_line, done) {
    ...
    html_parser.end();
    return stream.pipe($collect_texts()).pipe($hyphenate()).pipe($collect_tags()).pipe($split_texts()).pipe(D2.$show()).pipe($distribute_lines(test_line, send_line, done)).pipe(D2.$on_end((function(_this) {
