...
(function() {
  var $, CND, D, LODASH, LineBreaker, TEACUP, after, alert, badge, debug, echo, help, info, log, njs_fs, njs_path, rpr, sleep, step, suspend, urge, warn, whisper,
    __slice = [].slice;
  ...
  this._$hyphenate = function() {
    ...
    hyphenate = D.new_hyphenator.apply(D, P);
    return $((function(_this) {
      return function(event, send) {
        if (event[0] === 'text') {
  ...
  this.set_lines = (function(_this) {
    return function(text, test_line, accept_line, handler) {
      ...
      input.pipe(D.HTML.$parse()).pipe(D.HTML.$collect_texts()).pipe(D.HTML.$collect_empty_tags()).pipe(_this._$hyphenate()).pipe(_this._$break_lines()).pipe(_this._$disperse_texts()).pipe(_this._$produce_lines(state)).pipe(D.$show());
