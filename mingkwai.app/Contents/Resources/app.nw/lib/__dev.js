...
(function() {
  var $, $break_lines, $consume_lines, $convert_to_html, $hyphenate, $produce_lines, CND, D, D2, LINESETTER, LODASH, after, alert, badge, debug, echo, help, info, log, njs_fs, njs_path, prune_buffer, rpr, sleep, step, suspend, urge, warn, whisper,
    __slice = [].slice;
        ...
        switch (meta_type) {
          ...
          case 'set-line':
            ...
            for (_i = 0, _len = buffer.length; _i < _len; _i++) {
              ...
              switch (type) {
                case 'open-tag':
                  html += LINESETTER.render_open_tag.apply(LINESETTER, tail);
                ...
                case 'lone-tag':
                  ...
                  break;
                case 'empty-tag':
                  html += LINESETTER.render_empty_tag.apply(LINESETTER, tail);
                  break;
                case 'text-part':
            ...
            for (_j = 0, _len1 = open_tags.length; _j < _len1; _j++) {
              ...
              html += LINESETTER.render_close_tag(tag_name);
            }
  ...
  this.demo_3 = function() {
    ...
    typeset_text = (function(_this) {
      return function(text, test_line, accept_line, handler) {
        ...
        input.pipe(D2.HTML.$parse()).pipe(D2.HTML.$collect_texts()).pipe(D2.HTML.$collect_empty_tags()).pipe($hyphenate()).pipe($break_lines()).pipe(D2.$disperse_texts()).pipe(D2.$sub(function(source, sink, state) {
          return source.pipe($produce_lines(source, state)).pipe($convert_to_html()).pipe($consume_lines(state, text, test_line, accept_line, handler)).pipe(sink);
        }));
